                ...
                <div class="row">
                  ...
                  <div class="col-lg-12">
                    <div class="form-group">
                      ...
                      <div v-for="crude in product.crudes" :key="crude.id">
                        <crude
                          :crude="crude"
                          :productCrudes.sync="productCrudes"
                        />
                        <!--                                                <Crude :crude="crude"/>-->
                        <!--                                                <crude />-->
                      </div>
                      ...
                      >
                        ...
                        >
                          <span
                            class="multiselect__single"
                            v-if="values.length &amp;&amp; !isOpen"
                            >Выбрано размеров: {{ values.length }}</span
                          >
                        </template>
                      </multiselect>
                      <br />
...
//                 console.log(before)
//                 var cost = 0
//                 this.product.crudes.forEach(crude => cost += crude.price * crude.crude_qty)
...
export default {
  ...
  created() {
    this.product = JSON.parse(JSON.stringify(this.product_data));
    this.productCrudes = JSON.parse(JSON.stringify(this.product.crudes));
    this.productSizes = JSON.parse(JSON.stringify(this.product.sizes));
    // console.log(this.productCrudes)
    for (var i = 0; i < this.productCrudes.length; i++) {
      // console.log('qty')
      this.productCrudes[i].quantity = this.productCrudes[i].pivot.crude_qty;
    }
    // this.product.crudes.forEach(crude => crude.crude_qty = crude.pivot.crude_qty)
  },
  mounted() {
    // console.log(this.product)
    // this.sizes = [{'id': 1, 'size': 37}, {'id': 2, 'size': 38}, {'id': 3, 'size': 39}]
  ...
  watch: {
    ...
    productCrudes: function (val) {
      ...
      var cost = 0;
      val.forEach((element) => (cost += element.price * element.quantity));
      this.productCost = cost;
    },
    productSizes: function (val) {
      // console.log('productCrudes changed')
      this.product.sizes = val;
    },
    // "product.sizes": function (newVal, previousVal) {
    //     console.log(newVal)
    // }
  },
  methods: {
    ...
    update_record: function (id) {
      ...
      if (confirm("Вы уверены что хотите изменить запись?")) {
        ...
        axios
          .put(route("products.update", id), this.product)
          .then((response) => {
          ...
          if (files[i].status == "success") {
            ...
            if (files[i].name) {
              for (let j = 0; j < this.product.image.length; j++) {
                if (
                  files[i].name == this.product.image[j].fileType.name &&
                  files[i].size == this.product.image[j].fileType.size &&
                  files[i].type == this.product.image[j].fileType.type
                ) {
