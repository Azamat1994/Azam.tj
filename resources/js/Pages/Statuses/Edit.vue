                    ...
                    <div class="row justify-content-center">
                        <div class="col-12 col-lg-10 col-xl-8">
                            ...
                            <form class="mb-4">
                                        ...
                                        <div class="form-group float-right">
                                            ...
                                            <div class="input-group" style="cursor: pointer">
                                                <div class="input-group-prepend">
                                                    <span @click="status.is_active=1" :class="`input-group-text ${ status.is_active ? 'bg-primary text-white' : '' }`" id="popular">Да</span>
                                                </div>
                                                <div class="input-group-append">
                                                    <span @click="status.is_active=0" :class="`input-group-text ${ !status.is_active ? 'bg-primary text-white' : '' }`" id="not_popular">Нет</span>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <button type="button" class="btn btn-block btn-primary" @click="update_record(status.id)">
                                    Сохранить
                                </button>
    ...
    methods: {
        update_record: function (id) {
            if (confirm('Вы уверены что хотите изменить запись?'))
                axios.put(route("statuses.update", id), this.status)
                    .then(
                        (response) => {
                            this.$swal({title: response.data.message,timer: 3000})
                            this.$inertia.visit(route('statuses.index'));
                        },
                        (error) => {
