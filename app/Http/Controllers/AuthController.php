    ...
    public function store(Request $request)
    {
        ...
        if ($user) {
            return response()->json([
                ...
                'message' => 'Телефон или почта уже существуют. Если вы забыли пароль, попробуйте сбросить его по ссылке ниже...'
            ]);
        }
        $this->validate($request, [
            'name' => 'required',
...
//            }
            ...
            } else if ($request->country === 'tj') {
                $is_phone = is_numeric($request->login);
                if (!$is_phone) {
                    return response()->json([
                        'message' => 'Неправильный формат номера телефона'
                    ], 403);
            ...
            if ($request->country === 'other') {
                Mail::to($request->login)->send(new ConfirmMail($confirm));
            } else if ($request->country === 'tj') {
